...
export default function App() {
  ...
  return <div className="app-shell"><Header state={state} seed={seed} setSeed={setSeed} start={start} control={control} inject={inject} connected={connected} busy={!!busy}/>
    ...
    <main className="workspace"><AgentPanel agent="baseline" state={state?.agents.baseline} order={latest} simTime={state?.sim_time} evaluating={evaluating !== null} revision={state?.revision ?? 0}/><AgentPanel agent="smart" state={state?.agents.smart} order={latest} simTime={state?.sim_time} evaluating={evaluating !== null} revision={state?.revision ?? 0}/><OrdersFeed orders={state?.orders ?? []} shocks={state?.shocks ?? []} zones={zones} evaluating={evaluating}/></main>
