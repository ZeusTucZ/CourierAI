...
export interface AgentState { version: number; position: Coordinates; zone: number; status: string; action: string | null; net_earnings: number; current_order: string | null; pickup: Coordinates | null; dropoff: Coordinates | null; target: Coordinates | null; routes: Route[]; closures: FeatureCollection<MultiLineString>; completed: number }
export interface SimulationState { id: string; seed: number; status: 'preparing' | 'running' | 'paused' | 'updating' | 'completed' | 'error'; speed: number; error: string | null; revision: number; sim_time?: string; start_time?: string; end_time?: string; elapsed_seconds?: number; same_stream: boolean; provenance?: string; baseline_threshold?: number; agents: Partial<Record<AgentKey, AgentState>>; orders: Order[]; shocks: Shock[]; metrics?: Record<AgentKey, Metrics> | null }
