...
import type { AgentKey, AgentState, Coordinates, Order, Zone } from '../types'
...
export function AgentPanel({ agent, state, order, orders = [], zones = new Map(), simTime, evaluating, revision }: { agent: AgentKey; state?: AgentState; order?: Order; orders?: Order[]; zones?: Map<number, Zone>; simTime?: string; evaluating: boolean; revision: number }) {
  ...
  return <section className={`agent-panel ${agent}`} aria-label={smart ? 'Smart agent panel' : 'Baseline panel'}>
    ...
    {activeOrders.length > 0 && <div className="active-orders" aria-label={`${activeOrders.length} órdenes activas`}><div><span className="eyebrow">Órdenes activas</span><b>{activeOrders.length}</b></div><ul>{activeOrders.map((active, index) => <li key={active.order_id} className={active.is_current ? 'current' : ''}><strong>#{index + 1} · {active.order_id}</strong><span>{active.phase === 'to_pickup' ? 'Camino al restaurante' : active.phase === 'waiting' ? 'En el restaurante' : active.phase === 'to_dropoff' ? 'Camino al domicilio' : active.phase === 'delivering' ? 'Entregando' : 'Entregando'}</span></li>)}</ul></div>}
