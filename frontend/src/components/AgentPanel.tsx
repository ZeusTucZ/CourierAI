...
export function AgentPanel({ agent, state, order, orders = [], zones = new Map(), simTime, evaluating, revision }: { agent: AgentKey; state?: AgentState; order?: Order; orders?: Order[]; zones?: Map<number, Zone>; simTime?: string; evaluating: boolean; revision: number }) {
  ...
  return <section className={`agent-panel ${agent}`} aria-label={smart ? 'Smart agent panel' : 'Baseline panel'}>
    ...
    {activeOrders.length > 0 && <div className="active-orders" aria-label={`${activeOrders.length} órdenes activas`}><div><span className="eyebrow">Órdenes activas</span><b>{activeOrders.length}</b></div><ul>{activeOrders.map(active => <li key={active.order_id} className={active.is_current ? 'current' : ''}><strong>{active.order_id}</strong><span>{active.phase === 'to_pickup' ? 'En camino a recoger' : active.phase === 'waiting' ? 'Esperando preparación' : active.phase === 'to_dropoff' ? 'En camino a entregar' : 'En ruta'}</span></li>)}</ul></div>}
