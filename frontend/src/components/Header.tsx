...
export function Header({ state, seed, setSeed, start, control, inject, connected, busy }: { state: SimulationState | null; seed: number; setSeed: (seed: number) => void; start: () => void; control: (action: string, speed?: number) => void; inject: (type: Shock['shock_type']) => void; connected: boolean; busy: boolean }) {
  ...
  return <header><div className="topbar"><div className="brand"><div className="brand-mark">↗</div><div><h1>Courier AI <span>Simulator</span></h1><p>Monterrey, MX <span className="dot-separator">·</span> Same city. Different decisions.</p></div></div>
    ...
    <div className="controls"><label className="seed-control">SEED <input aria-label="Seed" type="number" min="0" max="2147483647" value={seed} disabled={busy || running} onChange={e => setSeed(Number(e.target.value))}/></label>
      ...
      <div className="playback"><button className="primary-button" onClick={start} disabled={busy || running || state?.status === 'completed'}>{busy ? 'Preparing…' : '▶ Start'}</button><button className="secondary-button" onClick={() => control('pause')} disabled={!running || busy}>Ⅱ Pause</button><button className="reset-button" onClick={() => control('reset')} disabled={!state || busy} aria-label="Reset simulation">↺ Reset</button></div>
