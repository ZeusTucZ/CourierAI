# MVP 3 — datos públicos y calibración
...
|---|---:|---:|---:|---:|
| zomato | 45584 | 34747 | 9106 | 1731 |
| order_history | 21321 | 17310 | 4011 | 0 |

Los tamaños utilizables son por campo: distancia/transiciones Zomato 31,654; duración Zomato 34,747; preparación Order History 16,993. No todas las filas tienen todos los atributos.
Solomon: C104, C204, R101, R201, RC104, RC204. Cada instancia aporta 100 clientes (600 total), además del depot auditado. C101, C201, RC101 y RC201 no están en el subconjunto clonado; se eligió el primer representante disponible de cada familia, del mismo repositorio.
...
| Parámetro | Estado | n |
|---|---|---:|
| pickup_distance_distribution | synthetic_fallback | 0 |
| order_distance_distribution | derived | 31654 |
| prep_time_distribution | observed | 16993 |
| service_time_distribution | observed | 34747 |
| base_pay_distribution | synthetic_fallback | 0 |
| tip_distribution | synthetic_fallback | 0 |
| weight_distribution | synthetic_fallback | 0 |
| volume_distribution | synthetic_fallback | 0 |
| orders_per_hour | synthetic_fallback | 51740 |
| zone_transition_distribution | derived | 31654 |
| time_of_day_demand_profile | synthetic_fallback | 0 |
| vehicle_distribution | observed | 34747 |
| multiple_delivery_frequency | observed | 33997 |
| base_reservation_wage | assumed | 0 |
| skip_penalty_mxn | assumed | 0 |
| cancellation_penalty_mxn | assumed | 0 |
| sla_time_multiplier | assumed | 0 |
| sla_buffer_min | assumed | 0 |
| operating_cost_per_km | assumed | 0 |
| sla_distance_buckets | derived | 31654 |

La tasa fallback es 15 ofertas/h; el n del manifest de esa tasa cuenta registros de entrada al fitter, no exposición ni una muestra válida para estimar la tasa. Demanda por hora permanece plana; `demand_structure.json` conserva frecuencias por hora observadas sin llamarlas tasas.
...
Cutoffs: Zomato 2022-03-28T23:59:59.999999; Order History 2024-12-31T23:59:59.999999. Particiones posteriores se guardan aparte y no participan en fit ni tuning.
...
Cobertura temporal reservada: 89.38% (n=753), 92.71% (n=1372), 91.89% (n=2047), 90.53% (n=4362). No se reajustó con estos resultados.
...
Elegido: 125 MXN/h. Es selección de un supuesto económico dentro de la simulación, no salario observado.
`artifacts/final_strategy_config.json`: public-mvp3-v1, perfil calibrated-b902d8308110, timestamp 2026-09-12T22:40:20.818496+00:00, hash dd10387758174c12f9e56a67b72f9f49b333d4661f6f318848a93edec82d7720. Contiene perfil, modelo y parámetros completos. Heldout verifica el hash y carga ese payload sin actualizarlo.
...
|---|---:|---:|
| Neto medio, MXN/shift | 997.09 | 1003.45 |
| Pedidos completados (total) | 104.00 | 103.00 |
| Entregas tardías (total) | 2.00 | 2.00 |
| Cancelaciones (total) | 0.00 | 0.00 |
| Distancia km (total) | 754.50 | 758.82 |
| Violaciones de seguridad | 0.00 | 0.00 |

Mejora media por shift: 0.7360%; mediana: 0.00%; Smart win rate: 20% (2 victorias, 7 empates, 1 derrota). Estado de seguridad: PASS. Cualquier violación no cero hace fallar el runner.
...
## Ablations (tuning seeds)

| Variante | mean_net | delta_vs_full | delta_vs_baseline |
|---|---:|---:|---:|
| SmartFull | 1007.69 | 0.00 | -5.53 |
| SmartNoZoneValue | 1007.69 | 0.00 | -5.53 |
| SmartNoReposition | 1007.69 | 0.00 | -5.53 |
| SmartNoImprovedBatching | 1007.69 | 0.00 | -5.53 |
| SmartNoHistoricalPrediction | 1007.69 | 0.00 | -5.53 |

No se ocultó el resultado negativo frente al baseline en tuning. No hay ventaja medida de estos componentes en este escenario. ZoneValue/Reposition/HistoricalPrediction no tienen señal económica observada para actuar; ImprovedBatching no altera el neto en estas seeds. No implica que esos componentes sean universalmente equivalentes.
...
Python 3.12 usado en esta ejecución; dependencias: `requirements-calibration.txt` y versiones efectivas en `artifacts/calibration_environment.txt`.
