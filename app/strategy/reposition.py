...
def choose_reposition(state, snapshot, policy, last_move=None, rain_factor=1):
    ...
    wait_rate = current.expected_net_mxn_per_hour if current else 0
    ...
    for zone, prediction in sorted(snapshot.zone_predictions.items()):
        ...
        gain = prediction.expected_net_mxn_per_hour * max(0, horizon - travel) / 60 - wait_rate * horizon / 60 - cost
