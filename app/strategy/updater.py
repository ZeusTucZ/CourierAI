...
class StrategyUpdater:
    ...
    def update(self, store, sim_time, vehicle, remaining_min, *, available=True):
        ...
        try:
            ...
            reference = mean(p.expected_net_mxn_per_hour for p in predictions.values()) if predictions else 0
            clip = self.policy.zone_value_clip
            values = {z: max(-clip, min(clip, p.expected_net_mxn_per_hour - reference)) for z, p in predictions.items()} if self.policy.use_zone_value else {}
            ...
            target = min(predictions, key=lambda z: (-predictions[z].expected_net_mxn_per_hour, z)) if predictions else None
